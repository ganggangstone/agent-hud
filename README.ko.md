...
[Agent Skills 명세](https://agentskills.io/specification)는 스킬 하나의 이름과 설명을
약 100토큰으로 본다. 이 도구를 만든 개발자의 컴퓨터에는 플러그인 3개로 스킬 60개가
...
- **플러그인 & 스킬**: 이 컴퓨터에 설치된 스킬 목록. 플러그인별로 나누고, 플러그인에
  ...
  켜기·끄기(켜짐/꺼짐 스위치)와 스킬 차단(허용/차단 스위치)도 할 수 있다. 두 기능은
  Claude Code에만 있으며, 스위치에 마우스를 올리면 Claude Code에만 적용된다는 설명이
  표시된다.
- **에이전트 지침**: 프로젝트 폴더에서 `CLAUDE.md`, `AGENTS.md`, `.clinerules`,
  `.cursor/rules/` 등 29종의 지침 파일을 찾아 크기와 수정 시각을 표시한다. 에이전트마다
...
플러그인을 내려받는 출처. git 저장소나 로컬 경로를 `claude marketplace add`로 등록한다.
...
각 탭 제목 아래 드롭다운에서 프로젝트를 선택한다. "+ 폴더 추가"로 훅 없이도 프로젝트를
추가할 수 있다.
...
클론한 폴더에서 `git pull`을 실행하고 서비스를 재시작한다.
